...
async def test_get_models(client: TestClient, mock_ollama_client: MagicMock):
...
async def test_switch_active_model_success(
    ...
    response = client.post(f"/api/v1/models/switch/{model_name}")
    assert response.status_code == status.HTTP_200_OK
    ...
    client.post("/api/v1/generate", json={"prompt": "test", "stream": False})
